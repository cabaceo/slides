---
author: "Binqi Ye and Harry Hua, prepared by Guangming Lang at Cabaceo LLC"
title: Shiny 
output: 
  ioslides_presentation:
    self_contained: no
    incremental: true
    fig_width: 7
    fig_height: 6
---

# Overview

## Goals | You will learn:

> - Shiny Fundamentals
> - Reactive Programming
> - Shiny Modules

## Outline

> - Hour 0.5: intro, usecases and setup.
> - Hour 1.5: shiny fundamentals (UI & server, input & output, and themes).
> - Hour 3  : reactive programming (the most important thing).
> - Hour 4  : modules (code re-use).


# Intro

## What?

> - Shiny allows you to rapidly build web apps using R.
> - [Shinyapps.io](https://www.shinyapps.io) allows you to easily deploy these apps.

## Who? 

> - Biostatisticians & Statisticians
> - Data scientists
> - R programmers

## Why? 

- Improve productivity. (Shiny allows you and your team to re-run calculation procedures by clicking buttons instead of running R scripts. It's easier to share an URL than a bunch of R files.)
- Let non-technical people use your models without knowing R. 
- Tell your data stories to the world. (Shiny supports powerful visualizations. Most shiny apps you make are mobile friendly by default.)
- Build minimum viable products.
- New career as Shiny developers.
- Not hard & fun.


# Demo

## Simple examples

1. Interactive plots. For example, a [bar chart](https://cabaceo.com/berkshire-vs-sp500/) that shows the actual values when you mouse over the bars. A [timeseries plot](https://cabaceo.com/shiller-pe/) that changes as you select different periods.

2. Turn your ideas into reality. For example, this [app](https://942kid.shinyapps.io/douban_explorer/) allows you to look up a user's most recent readings on Douban (豆瓣). Notice you can also embed google ads in the app. This [app](https://cabaceo.shinyapps.io/tabelog-en/) provides a cleaner interface to search restaurants on Tabelog in English. 

3. Embed data analysis in your blog post so that readers can play with the results. [Example](https://uasnap.shinyapps.io/ex_leaflet/).

## Complex examples

1. Super easy to run calculations under different scenarios or statistical procedures with different input parameters. For example, [calculate sample sizes in multi-regional clinical trials](https://gmlang.shinyapps.io/SSCRC/).

2. Train and backtest various models, compare results and generate reports. For example, here's an [app](http://hdnom.io) that does all of that for survival models.

3. You can build an entire product/SaaS platform using Shiny. [Example](https://newsroom.ly).

4. [More](https://shiny.rstudio.com/gallery/) [examples](https://www.showmeshiny.com).

## What again?

Shiny = statistical (R) programming + web technologies. 

It allows statisticians who don't know web technologies to easily make and publish interactive web apps. 

# Setup

## Install dev tools

> - [R](https://www.r-project.org)
> - [Rstudio](https://www.rstudio.com/products/rstudio/download/)
> - Web broswer (Safari, Chrome or Firefox)
> - Sign up for a FREE account at [shinyapps.io](http://www.shinyapps.io)

## Change Rstudio layout

> - Go to **Rstudio -> Preferences -> Pane Layout**
> - Change to as shown below. 

<img src="img/pane-layout.png" alt="" style="width: 55%;"/>

## Change tab width

> - Default is 2 spaces. Change to 8.
> - This will force you to write shorter code.
<img src="img/tab-width.png" alt="" style="width: 60%;"/>

## Show margin line

> - Check **Show margin** and set **Margin column** to be 80.
> - When writing code, never go beyond this margin line. 
> - This will make your code easier to read.
<img src="img/margin-col.png" alt="" style="width: 50%;"/>

## R code style

When naming, 

> - names should be meaningful
> - use lower case 

```{r, eval=FALSE}
# Good: 
student <- 25
foo.R, 

# Bad: because of Uppercased 1st letter
Student <- 25
Foo.R
```

## R code style cont.

> - use _ to connect

```{r, eval=FALSE}
# Good: 
learn_shiny <- function(start, end)

# Bad: because of camelCase 
learnShiny<- function(start, end)
```

> - use verb to name a function

[R Style Guide](http://style.tidyverse.org/files.html)

## Install R packages

```{r, eval=FALSE}
pkgs = c("shiny", "shinydashboard", "tidyverse", "plotly")
for (pkg in pkgs) {
        if (!pkg %in% installed.packages()) {
                cat(paste(pkg, "missing, will attempt to install\n"))
                install.packages(pkg)
        } else cat(paste(pkg, "installed OK\n"))
}
```

## Hello, World!

Open <font color="blue">examples/00-hello-world-boring.R</font> in Rstudio.

> - Click the arrow next to **Run App** -> check **Run External** (This will open the app in browser.)

<img src="img/open-in-browser.png" alt="" style="width: 60%;"/>

Click **Run App**.

## Read code

```{r, eval=FALSE}
library(shiny)

# make UI ---
ui <- fluidPage(
        titlePanel("Hello World!")
)

# make server ---
server <- function(input, output) {}

# make Shiny app ----
shinyApp(ui = ui, server = server)
```

## Introduce yourself

> - Add an input box to let people enter their names. 
> - Output a sentence to say hello to the world.

Run <font color="blue">examples/01-hello-world.R</font>

## Read code

```{r, eval=FALSE}
# make UI ---
ui <- fluidPage(
        titlePanel("Hello World!"),
        ###<b>
        mainPanel(
                textInput("my_name", 
                          label = "Enter your name:", 
                          value = "Harry"),
                textOutput("self_intro")
        )
        ###</b>
)

# make server ---
server <- function(input, output) {
        ###<b>
        output$self_intro = renderText(
                paste("My name is", input$my_name)
        )
        ###</b>
}
```

## Use sidebar 

Run <font color="blue">examples/01-hello-world-w-sidebar.R</font>

```{r, eval=FALSE}
ui <- fluidPage(
        titlePanel("Hello World!"),
        ###<b>
        sidebarLayout(
                sidebarPanel(
                        textInput("my_name", 
                                  label = "Enter your name:", 
                                  value = "Harry")
                        ),
                
                mainPanel(
                        textOutput("self_intro")
                )
        )
        ###</b>
)
```


## Exercise 1

> - Make a shiny app that draws a histogram of some made-up data, letting users specify the number of bins.

Complete <font color = "red">exercises/01-plot-histogram.R</font> together.


## Basic structure of Shiny App

### UI:
- fluidPage()
    - sidebarLayout()
        - sidebarPanel()
        - mainPanel()

### server:
- renderText()
- renderPlot()

## UI 

### Input elements (inside of **sidebarPanel()**):
- textInput()
- numericInput()

### Output elements (inside of **mainPanel()**):
- textOutput()
- plotOutput()

## server
```{r, eval=FALSE}
server <- function(input, output) {
        1. takes user supplied values from input
        2. run computation
        3. return or display results through output
}
```

## Break (5 minutes)

## Exercise 2

> - Make a shiny app that draws a histogram of Petal.Length in the iris data. Use a slider input to let user choose the number of bins.
> - ?sliderInput()
> - 10 minutes

## Exercise 3

> - Add a scatter plot of Sepal.Length vs Sepal.Width. Add a text input box to let user supply a plot title. 
> - 10 minutes

## Deploy Shiny App

> - Click the blue button.

<img src="img/deploy.png" alt="" style="width: 100%;"/>

# Bigger Shiny Apps 

## File Structures

<img src="img/structure-complex.png" alt="" style="width: 80%;"/>

## File Structures

> - global.R
> - ui.R
> - server.R

## Example global.R

```{r, eval=FALSE}
library(shiny)
library(shinyjs)
library(dplyr)
library(ggplot2)

# change max upload file size to 30 MB, default is 5 MB
options(shiny.maxRequestSize = 30*1024^2, shiny.reactlog = T)

# set paths
helper_path = "R/helper"
ui_path = "R/ui"
server_path = "R/server"

# load helper functions
for (fname in list.files(helper_path)) 
        source(file.path(helper_path, fname))
```

## Example ui.R
```{r, eval=FALSE}
navbarPage(
        theme = "darkly1.css", # look for the file in /www 
        title = 'DataTable Options',
        tabPanel('Show 15 rows per page', DT::dataTableOutput('ex1')),
        tabPanel('All rows on one page',  DT::dataTableOutput('ex3')),
        tabPanel('No search box',         DT::dataTableOutput('ex4')),
        tabPanel('with JavaScript',       DT::dataTableOutput('ex5'))
)
```

## Template server.R
```{r, eval=FALSE}
server <- function(input, output) {
        1. takes user supplied values from input
        2. run computation
        3. return or display results through output
}
```

## An example with css and js

<img src="img/struct-complex-example.png" alt="" style="width: 100%;"/>

Run <font color="blue">examples/02-output-data-table/ui.R</font>

Look at the code in global.R, ui.R and server.R

## Break (3 minutes)



# Reactive Programming

## Exercise

Open <font color="blue">exercises/04-reactivity-00.R</font>. 

Task: complete the server function. 
Make the plot output show a simple plot of the first nrows of a built-in dataset. 
If you can't think of any, use `cars`. So basically, make the Shiny equivalent of 
`plot(head(cars, nrows))`.

> - 2 minutes

<font size=2>credit: [Joe Cheng](https://www.rstudio.com/resources/videos/effective-reactive-programming/)</font>

## Solution

```{r, eval=FALSE, results="asis"}
output$plot <- renderPlot({
        plot(head(cars, input$nrows))
})
```

Run the app. As we change the number of rows, the scatter plot updates automatically. 
Input changes and output changes accordingly. Let's look at another example of
reactivity.

## Example: Fibonacci

This app finds the nth Fibonacci number and its inverse. 

I have two versions. One is slow and the other is faster. This is because the way 
I used reactivitiy are different. 

Run <font color="blue">examples/03-reactivity-fibonacci-slow.R</font>. 

Run <font color="blue">examples/03-reactivity-fibonacci-fast.R</font>. 

<font size=2>credit: [Reactivity - An overview](https://shiny.rstudio.com/articles/reactivity-overview.html)</font>

## Fibonacci Code Digest

### Slow Version
```{r, eval=FALSE}
server <- function(input, output) {
        ###<b>
        # run fib() twice 
        output$nth_fib <- renderText({ fib(input$n) })
        output$nth_fib_inv <- renderText({ 1 / fib(input$n) })
        ###</b>
}
```

### Fast Version
```{r, eval=FALSE}
server <- function(input, output) {
        ###<b>
        # only run fib() once
        current_fib <- reactive({ fib(input$n) })
        ###</b>
        output$nth_fib <- renderText({ current_fib() })
        output$nth_fib_inv <- renderText({ 1 / current_fib() })
}
```

## You may ask

Q: In the fast version, although it runs `fib()` only once, it runs
   `current_fib()` twice. Isn't this the same as if you run `fib()` twice?

```{r, eval=FALSE}
server <- function(input, output) {
        # only run fib() once
        current_fib <- reactive({ fib(input$n) })
        ###<b>
        output$nth_fib <- renderText({ current_fib() })
        output$nth_fib_inv <- renderText({ 1 / current_fib() })
        ###</b>
}
```

A: No. We are NOT doing function composition here because `current_fib()` is not 
really a function, although we use it as if it is.

## Subtle but important difference

This question shows that we misunderstood reactivity. It shows that we viewed 
`reactive({})` as a function. We thought we were pushing `fib(input$n)` into 
`reactive({})`, that we got a result returned back, and that we were 
assigning the result to `current_fib`. **This is wrong.**

Instead of thinking `reactive({})` as a function that performs `what`, think of it 
as a recipe that describes `how`.

In other words, when using expressions like `reactive({})`, `renderPlot({})`, and
`renderText({})`, we're not telling R to do this or that. Instead, we're
describing how to do something and let R decide when to do it or if to do it 
at all.

## A moment of clarity

There is a difference between telling Shiny *to do* something, and telling Shiny
*how to do* something. 

### Maxim

There is a difference between telling people *to do* something, and showing 
people *how to do* something. 

### Corollary 1

The next time someone tells you to do something, ask *how*. 

### Corollary 2

Ignore the people who always tell you *what to do* but can't show you *how to do* it. 

## Really understand reactivity

Let's consider a very basic app.

Run <font color="blue">examples/04-reactivity-show-number.R</font>. 

Here's the server logic. It just shows the input number on the screen.

```{r, eval=FALSE}
server <- function(input, output) {
        output$text <- renderText({
                print(input$a)
        })
}
```

How does this work under the hood?

## How do you print the value of a variable in R?
```{r, eval=FALSE}
a = 50
print(a)

a = 75 # this action makes print(a) "out of date"
```

After a is printed,  it's given a new value of 75, how do you print this new value of a?

## Call `print(a)` again.

In general, an expression is called **out of date** if one or more objects in the 
expression has been given a new value since the expression was last called. To 
**update** an out-of-date expression is easy, just re-run the expression. This is 
just standard R stuff, not reactivity.

In theory, we don't need shiny. We can build the app from ground zero by 
continuously re-running every expression in the app. Information are **pulled**
from the input instead of **pushed** to the output. For example, `print(input$a)` 
learns the new value of `input$a` because the server re-runs `print(input$a)`, 
not because the new value is magically pushed into output.

In practice, this approach will quickly slow down your app and make it unresposive.

## The key to reactivity

> Shiny solves this problem by creating a **system of alerts** that lets the server know when an expression becomes **out of date**. The server still checks in on your app every few microseconds, but instead of re-running each expression each time, it only runs the expressions that the alert system has flagged as **out of date**. If no alerts have appeared, the server does not have to run anything at all. It can rest until the next check. If alerts have appeared, the server runs all of the expressions that are **out of date** at that moment, an event known as a **flush**.
> 
> <cite>[How to understand reactivity in R](https://shiny.rstudio.com/articles/understanding-reactivity.html)


## Alerts are like carrier pigeons

<img src="img/pigeon1.png" alt="" style="width: 100%;"/>

<font size=2>credit: [Rstudio](https://shiny.rstudio.com/articles/understanding-reactivity.html)</font>

## 

<img src="img/pigeon2.png" alt="" style="width: 100%;"/>

<font size=2>credit: [Rstudio](https://shiny.rstudio.com/articles/understanding-reactivity.html)</font>

## 

<img src="img/pigeon3.png" alt="" style="width: 100%;"/>

<font size=2>credit: [Rstudio](https://shiny.rstudio.com/articles/understanding-reactivity.html)</font>


## Revisit Fibonacci Fast Version

Will this work? Why and why not?

```{r, eval=FALSE}
server <- function(input, output) {
        ###<b>
        current_fib <- fib(input$n)
        ###</b>
        output$nth_fib <- renderText({ current_fib() })
        output$nth_fib_inv <- renderText({ 1 / current_fib() })
}
```

- Let's find out. Run <font color="blue">examples/05-reactivity-fib-fast-revisit.R</font>. 
- Read the error message.
- We forgot to tell Shiny this is a recipe. To fix, wrap `fit(input$n)` inside of 
`reactive({})`: `current_fib <- reactive({ fib(input$n) })`.


## Exercise 1

Open <font color="blue">exercises/04-reactivity-01.R</font>. 

Task: re-write server logic to make sure that `head()` is only run once for each
change to `input$nrows`.

>- 5 minutes

<font size=2>credit: [Joe Cheng](https://www.rstudio.com/resources/videos/effective-reactive-programming/)</font>

## Solution

```{r, eval=FALSE}
server <- function(input, output) {
        ###<b>
        df <- reactive({
                head(cars, input$nrows)
        })
        ###</b>
        
        output$plot <- renderPlot({
                plot(df())
        })
        
        output$table <- renderTable({
                df()
        })
}
```

- How many of you get something like this?
- Any of you did something different?

## Exercise 2

Open <font color="blue">exercises/04-reactivity-02.R</font>. 

Run it and it works. The problem is that each of the 4 outputs contains 
copied-and-pasted logic for selecting your chosen variables, and for building
the model. 

Task: Can you refactor the code so it's more efficient and maintainable?

>- 10 minutes

<font size=2>credit: [Joe Cheng](https://www.rstudio.com/resources/videos/effective-reactive-programming/)</font>


## Solution

```{r, eval=FALSE}
selected <- reactive({
        iris[, c(input$xcol, input$ycol)]
})

model <- reactive({
        lm(paste(input$ycol, "~", input$xcol), selected())
})
```

- How many of you get something like this?
- Any of you did something different?

## You can chain reactive expressions!
```{r, eval=FALSE}
###<b>
selected <- reactive({
        iris[, c(input$xcol, input$ycol)]
})
###</b>

model <- reactive({
        lm(paste(input$ycol, "~", input$xcol), 
           ###<b>
           selected()
           ###</b>
           )
})
```







# Modules

## Placeholder



# Where to go next?

## Resources

> - [R style guide](http://style.tidyverse.org).
> - Rstudio's guided [tutorial](https://shiny.rstudio.com/tutorial/) for beginners.
> - Rstudio's technical [articles](https://shiny.rstudio.com/articles/).
> - [Shiny github page](https://github.com/rstudio/shiny).
> - [Shiny Google group](https://groups.google.com/forum/#!forum/shiny-discuss).
> - [>100 shiny app examples with code](https://github.com/rstudio/shiny-examples).
> - [Shiny developer conference videos 2016](https://www.rstudio.com/resources/webinars/shiny-developer-conference/).
